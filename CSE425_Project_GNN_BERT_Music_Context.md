...
|---|---|---|---|
| FMA (small / medium) | Genre (8–16 classes), top tags, artist & album metadata | 1–3 | [FMA Download](https://github.com/mdeff/fma) |
| MagnaTagATune | 188 multi-label tags (genre, mood, instrument); 25,877 clips | 1, 3 | [MagnaTagATune](https://mirg.city.ac.uk/codeapps/the-magnatagatune-dataset) |
| GTZAN | 10 genres; 1,000 tracks (30s clips); standard easy baseline | 1–2 | [GTZAN Download](https://www.kaggle.com/datasets/andradaolteanu/gtzan-dataset-music-genre-classification) |
| DEAM | Continuous valence & arousal (1–9) every 0.5s | 3–4 | [DEAM](https://cvml.unige.ch/databases/DEAM/) |
| MusicCaps | 5,521 clips with expert natural-language captions (Google) | 1, 4 | [MusicCaps](https://research.google/resources/datasets/musiccaps/) |
| Million Song Dataset + tagtraum | Last.fm-style tags for 188,916 tracks (Echo Nest audio features) | 1, 3 | [MSD Download](http://millionsongdataset.com/) |
| Lakh MIDI Clean | Symbolic MIDI for chord-transition & melody graph construction | 2–3 | [LMD Download](https://colinraffel.com/projects/lmd/) |
| EmoMusic | 1,000 clips with quadrant emotion labels (happy/sad × calm/energetic) | 3 | [EmoMusic](https://cvml.unige.ch/databases/emoMusic/) |

*Task key: 1 = BERT tags — 2 = GNN structure — 3 = GNN–BERT fusion — 4 = contrastive retrieval. Use ≥ 1 audio dataset and ≥ 1 text/tag source.*

> **Note:** The links above are the standard official/reference sources for each dataset. Please verify each link before use, as dataset hosting locations can change over time.
...
```
A = softmax( QK^T / √d ),   Q = gW_Q,   K = H_text W_K
...
**Require:** Tag corpus D = {(X_text^(i), y^(i))}; Pretrained BERT, learning rate η, epochs E
...
2: g ← GNN_Readout(G)
3: z ← CrossAttention(g, H_text)   ▷ or CONCAT(g, t)
4: ŷ ← σ(Wz + b)
5: L ← L_tags + α·L_emotion
6: Backprop through fusion, GNN, and (partial) BERT
```
...
4:     Compute similarity matrix S_ij = g_i^T t_j / τ
5:     L_NCE ← −(1/N) Σ_i log( exp(S_ii) / Σ_j exp(S_ij) )
...
- **IEEE Conference:** [https://www.overleaf.com/latex/templates/ieee-conference-template](https://www.overleaf.com/latex/templates/ieee-conference-template)
- **ICML 2025:** [https://www.overleaf.com/latex/templates/icml2025-template](https://www.overleaf.com/latex/templates/icml2025-template)
